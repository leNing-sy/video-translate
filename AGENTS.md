...
- 默认触发 GitHub Actions 等云端构建后不等待、不持续轮询，以节省本地资源和对话额度；触发成功后记录运行链接、运行编号、分支或提交，并继续其他工作。
- 只有用户明确要求等待、监视、下载或测试某次构建时，才持续跟进该次运行；检查间隔不低于合理的异步等待周期，单次不超过 60 秒。
...
- **Ollama** - LLM 运行时服务（字幕翻译必需；仅提取原文时可不安装，应用按需管理）
- **Node.js 22.13+** - 开发与构建运行时
...
- 临时文件自动清理
- 实施了 Electron 安全最佳实践
- 默认在本地处理；启用 BYOK 润色时会调用用户配置的 OpenAI 兼容接口
