...
- 🧵 **并发队列** - 默认最多同时运行 3 个任务，超出部分自动排队，避免占满 CPU、内存及 ASR / FFmpeg 资源
...
- 主任务默认最多同时运行 **3 个**；第 4 个及之后的任务会自动进入队列。
- 任务完成、失败或取消后，队列会自动补位；同一任务在运行或排队期间不会重复启动。
- 暂停运行中的任务后，如果立即点击恢复，应用会等待原运行实例完全退出，再启动唯一的新实例，不会同时运行同一任务的多个实例。
- 删除任务或应用退出时，排队项会被取消，旧运行实例也不会再把进度、日志或结果写回已删除/新运行的任务。
- 该限制用于控制本机 CPU、内存以及 ASR、FFmpeg 等资源占用；实际耗时仍取决于视频时长、模型和硬件配置。
...
并发调度实现位于 `apps/desktop/src/main/services/task-run-coordinator.ts`，对应的生命周期回归测试为 `apps/desktop/src/main/services/task-run-coordinator.test.ts`。
